...
public class _JUtil
{
    public static void CreateSphereDirectShape(XYZ center, double radius, Document doc)
    {
    ...
    double tolerance = _eps)
    {
        var p1 = new XYZ(Math.Abs(p.X), Math.Abs(p.Y), Math.Abs(p.Z));
        var q1 = new XYZ(Math.Abs(q.X), Math.Abs(q.Y), Math.Abs(q.Z));
        int d = Compare(p1, q1, tolerance);  
        return d;
    }
    ...
    public static XYZ[] GetPointsFromBoundingBox(BoundingBoxXYZ bbox)
    {
        XYZ pt0 = new XYZ(bbox.Min.X, bbox.Min.Y, bbox.Min.Z);
        XYZ pt1 = new XYZ(bbox.Max.X, bbox.Min.Y, bbox.Min.Z);
        XYZ pt2 = new XYZ(bbox.Max.X, bbox.Max.Y, bbox.Min.Z);
        XYZ pt3 = new XYZ(bbox.Min.X, bbox.Max.Y, bbox.Min.Z);
        XYZ pt4 = new XYZ(bbox.Min.X, bbox.Min.Y, bbox.Max.Z);
        XYZ pt5 = new XYZ(bbox.Max.X, bbox.Min.Y, bbox.Max.Z);
        XYZ pt6 = new XYZ(bbox.Max.X, bbox.Max.Y, bbox.Max.Z);
        XYZ pt7 = new XYZ(bbox.Min.X, bbox.Max.Y, bbox.Max.Z);
        return new XYZ[] { pt0, pt1, pt2, pt3, pt4, pt5, pt6, pt7 };
    }

    public static bool IsBoundingBoxexIntersect(BoundingBoxXYZ bb1 , BoundingBoxXYZ bb2)
    {
        var points = GetPointsFromBoundingBox(bb2); 
        return points.Any(x => BoundingBoxXyzContains(bb1, x));
    }
...
public static class JtPlaneExtensionMethods
{
    ...
    {
        Debug.Assert(
          _JUtil.IsEqual(plane.Normal.GetLength(), 1),
      ...
      XYZ p)
    {
        ...
        Debug.Assert(
          _JUtil.IsZero(plane.SignedDistanceTo(q)),
