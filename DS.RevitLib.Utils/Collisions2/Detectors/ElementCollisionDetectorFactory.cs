using Autodesk.Revit.DB;
using System.Collections.Generic;
using System.Linq;

namespace DS.RevitLib.Collisions2
{
    /// <summary>
    /// An object to detect collisions (intersections) between <see cref="Autodesk.Revit.DB.Element"/>
    /// and ohter <see cref="Autodesk.Revit.DB.Element"/>'s in Revit model. 
    /// <para>It uses <see cref="Autodesk.Revit.DB.BoundingBoxIntersectsFilter"/> by each <see cref="Autodesk.Revit.DB.Element"/> to detect collisions faster.</para>
    /// </summary>
    public class ElementCollisionDetectorFactory : CollisionDetectorFactory<Element, Element>
    {
        private readonly Document _doc;
        private readonly ElementCollisionDetector _modelDetector;
        private readonly List<ElementCollisionDetector> _linkDetectors;

        /// <summary>
        /// Instantiate an object to detect collisions in <see cref="Document"/>.
        /// </summary>
        public ElementCollisionDetectorFactory(Document doc,
            List<Element> docElements, Dictionary<RevitLinkInstance, List<Element>> linkElementsDict = null)
        {
            _doc = doc;
            _modelDetector = new ElementCollisionDetector(_doc, docElements);
            _linkDetectors = linkElementsDict is null ? null : GetLinkDetectors(linkElementsDict);
        }

        /// <summary>
        /// Get collisions between <paramref name="checkObject1"/> and objects in 
        /// <see cref="Document"/> and all loaded <see cref="RevitLinkInstance"/>'s.
        /// </summary>
        /// <param name="checkObject1"></param>
        /// <param name="checkObjects2ToExclude"></param>
        /// <returns>Returns collisions list. Returns empty list if no collisions were detected.</returns>
        public override List<IBestCollision> GetCollisions(Element checkObject1, List<Element> checkObjects2ToExclude = null)
        {
            Collisions = new List<IBestCollision>();

            //get colliisons in model
            var modelCollisions = _modelDetector.GetCollisions(checkObject1, checkObjects2ToExclude).Cast<ElementCollision>().ToList();
            Collisions.AddRange(modelCollisions);

            //get colliisons in links
            if (_linkDetectors is not null && _linkDetectors.Any())
            {
                foreach (var linkDetector in _linkDetectors)
                {
                    var linkCollisions = linkDetector.GetCollisions(checkObject1, checkObjects2ToExclude).Cast<ElementCollision>().ToList();
                    Collisions.AddRange(linkCollisions);
                }
            }

            return Collisions;
        }

        private List<ElementCollisionDetector> GetLinkDetectors(Dictionary<RevitLinkInstance, List<Element>> linkElementsDict)
        {
            var detectors = new List<ElementCollisionDetector>();

            //get colliisons in links
            if (linkElementsDict is not null && linkElementsDict.Any())
            {
                foreach (var item in linkElementsDict)
                {
                    var linkDetector = new ElementCollisionDetector(item.Key, item.Value);
                    detectors.Add(linkDetector);
                }
            }

            return detectors;
        }
    }
}
